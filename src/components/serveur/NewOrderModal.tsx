...
// Modal de prise de commande pour table libre
...
export function NewOrderModal({
  ...
  table,
  onOrderCreated,
}: NewOrderModalProps): JSX.Element | null {
  ...
  const { createOrder } = useServerOrders();
  ...
  const handleSubmit = async () => {
    ...
    try {
      const orderId = await createOrder(table.id, items, customerName || undefined);
      clearCart();
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-outline-variant/10">
          ...
          <input
            ...
            placeholder="Ex: Table de M. Dupont"
            className="w-full max-w-md bg-surface-container-highest text-on-surface placeholder-on-surface-variant/50 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-container"
          />
        </div>
          ...
          <div className="border-t border-outline-variant/10 bg-surface-container-high">
            ...
            <div className="px-6 py-4 bg-surface-container-highest">
              ...
              <div className="flex gap-3">
                ...
                >
                  {isSubmitting ? 'Envoi...' : 'VALIDER COMMANDE'}
                </button>
              </div>
            </div>
          </div>
        ...
        {items.length === 0 && (
          <div className="px-6 py-8 border-t border-outline-variant/10 bg-surface-container-high">
            <div className="text-center">
              ...
              <p className="text-on-surface-variant text-sm">
                Sélectionnez des items dans le menu pour commencer la commande
              </p>
            </div>
          </div>
        )}
